...
Readiness assessment, 2026-09-24: the modified official CM4 image now has local
...
| --- | --- | --- | --- |
| M0: installation repair | Workbench installs firmware, native flashing tools and editable firmware source | Install the actual archive into a clean prefix, launch outside the checkout as an ordinary user, edit/save source, and verify bundled firmware provenance and freshness | Actual Linux ARM64 archive launch/edit/save and firmware provenance pass; remaining host/release qualification is open |
| M1: developer preview | Repeatable official-image desktop through Xorg/fbdev | Close the hardening items below; fresh-overlay onboarding, reboot, keyboard/mouse interaction, application launch and clean shutdown pass; package tests pass on each advertised host | Linux ARM64 onboarding, explicit stock Desktop-path repair, mouse-launched Terminal, typed command, reboot and normal systemd shutdown demonstrated with a clean stopped root superblock; final installed-artifact host matrix and other hardening gates remain |
| M2a: dual-target image forge | Persistent image modification, checkpoints and hardware-compatible export | One modified exported image passes the IDE/hardware/reimport loop above; native boot defaults survive; updated kernel artifacts are refreshed; interrupted operations recover safely | Shared-runtime app/service/module modification, checkpoint restoration, export/reimport, fresh service restart and normal shutdown pass with unchanged native boot-file hashes; interactive desktop loop and physical dual-target qualification remain |
| M2b: agent-enabled stable forge | Shared headless controller, CLI, MCP adapter and companion skill | GUI/CLI/MCP parity, workspace isolation, concurrent-client safety, permission boundaries, job cancellation and an external-agent edit/test/export exercise pass; supported host matrix is explicit | Private runtime, permission-scoped MCP, packaged skill and durable history pass locally; GUI boot, desktop preparation, maintenance shutdown, image/transfer/guest/approved-host tasks, pause/resume and power operations/replay use shared jobs with real acceptance evidence; live cross-client coordination and complete external-agent/host acceptance remain |
...
* Build and exercise release artifacts on the advertised hosts. Require GUI
  tests and emulator device tests in the release gate; the current release
  script runs headless checks and packaging only. Record fresh CI for the
  release revision, checksums and known limitations before publication.
