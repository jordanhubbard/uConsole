...
| M2a: dual-target image forge | Persistent image modification, checkpoints and hardware-compatible export | One modified exported image passes the IDE/hardware/reimport loop above; native boot defaults survive; updated kernel artifacts are refreshed; interrupted operations recover safely | IDE enhancement/export/bootback and clean derivative checks pass. Physical unchanged-source streaming, persistent RAM hold/release, normal return and nine-preimage restoration pass. Enhanced-root physical deployment, rollback and final reimport remain open |
| M2b: agent-enabled stable forge | Shared headless controller, CLI, MCP adapter and companion skill | GUI/CLI/MCP parity, workspace isolation, concurrent-client safety, permission boundaries, job cancellation and an external-agent edit/test/export exercise pass; supported host matrix is explicit | GUI-attached external-client editing/export, competing-client controls, cancellation and retained history have evidence. Owner-pinned recovery jobs and boot-bound staging/reconciliation are exposed through Workbench/MCP; real stdio MCP staged all four physical boot phases. Final integrated image/hardware and host-matrix acceptance remain |
